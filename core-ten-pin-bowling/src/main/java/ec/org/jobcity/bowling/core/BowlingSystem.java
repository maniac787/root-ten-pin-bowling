...
public class BowlingSystem {
    ...
    public void bowlFrame(Player player, int frame) throws IOException {
        ...
        int score = getScore(player, totalScore);
        ...
        if (totalScore == 10) { // Checks for strike
            ...
            score = getScore(player, totalScore);
     ...
     */
    public int getScore(Player player, int totalScore) throws IOException {
        boolean validScore = false;
        int tempScore = 0;
        while (validScore == false) {
            String temp = player.get_score().toString();
            while (temp.equals("")) {
     ...
     */
    public int getScore(Player player) throws IOException {
        boolean validScore = false;
        int tempScore = 0;
        while (validScore == false) {
            String temp = player.get_score().toString();
